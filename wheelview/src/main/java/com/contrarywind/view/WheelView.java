...
//                canvas.scale(1.0F, (float) Math.sin(radian));
    ...
    private void reMeasureTextSize(String contentText) {
        ...
        int width = rect.width()+getResources().getDimensionPixelOffset(R.dimen.center_text_padding);
        int size = textSize;
        while (width > measuredWidth) {
            ...
            paintCenterText.getTextBounds(contentText, 0, contentText.length(), rect);
            width = rect.width()+getResources().getDimensionPixelOffset(R.dimen.center_text_padding);
        }
        //设置2条横线外面的文字大小
