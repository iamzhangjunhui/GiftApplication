...
public class WheelOptions<T> {
    ...
    private boolean twoLinkage;//是否前两项联动
    ...
    //前两个联动后两个不联动情况下
    public void set2Picker(List<T> options1Items, final List<List<T>> options2Items, List<T> options3Items, List<T> options4Items) {
        ...
        if (options2Items != null) {
            this.mOptions2Items = options2Items;
            wv_option2.setAdapter(new ArrayWheelAdapter(options2Items.get(0)));// 设置显示数据
            wv_option2.setCurrentItem(wv_option2.getCurrentItem());// 初始化时显示的数据
        }
        // 选项3
        if (options3Items != null) {
            ...
            public void onItemSelected(int index) {
                ...
                if (options2Items == null) {
                    ...
                    if (!isRestoreItem) {
                        ...
                        opt2Select = opt2Select >= options2Items.get(index).size() - 1 ? options2Items.get(index).size() - 1 : opt2Select;
                    }
                    wv_option2.setAdapter(new ArrayWheelAdapter(options2Items.get(index)));
                    wv_option2.setCurrentItem(opt2Select);
                    if (optionsSelectChangeListener != null) {
                        optionsSelectChangeListener.onOptionsSelectChanged(index, opt2Select, wv_option3.getCurrentItem(), wv_option4.getCurrentItem());
                    }
                }
            }
        });
        ...
        //添加联动监听
        if (options2Items != null && twoLinkage)
            wv_option2.setOnItemSelectedListener(wheelListener_option2);
    ...
    public int[] getCurrentItems() {
        ...
        if (mOptions4Items != null && mOptions4Items.size() > 0) {//非空判断
            currentItems[3] = wv_option4.getCurrentItem() > (mOptions4Items.get(currentItems[0]).get(currentItems[1]).size() - 1) ? 0 : wv_option4.getCurrentItem();
        } else {
    ...
    public void setCurrentItems(int option1, int option2, int option3, int option4) {
        if (linkage) {
            ...
            if (twoLinkage) {
                wv_option2.setAdapter(new ArrayWheelAdapter(mOptions2Items.get(option1)));
    ...
    public void setTwoLinkage(boolean twoLinkage) {
