...
public class WheelTime {
    ...
    private static final int DEFAULT_START_MINUTED = 0;
    private static final int DEFAULT_END_MINUTED = 59;
    ...
    private int startMinute = DEFAULT_START_MINUTED;
    private int endMinute = DEFAULT_END_MINUTED;
     ...
     * @param s
     */
    private void setSolar(int year, final int month, final int day, int h, int m, int s) {
        ...
        if (startYear == endYear && startMonth == endMonth && startDay == endDay && startHour == endHour && startMinute == endMinute) {
            wv_seconds.setAdapter(new NumericWheelAdapter(startSecond, endSecond));
            wv_seconds.setCurrentItem(s - startSecond);
        } else if (year == startYear && month == startMonth && day == startDay && h == startHour && m == startMinute) {
            wv_seconds.setAdapter(new NumericWheelAdapter(startSecond, 59));
            wv_seconds.setCurrentItem(s - startSecond);
        } else if (year == endYear && month == endMonth && day == endDay && h == endHour && m == endMinute) {
        ...
        wv_year.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                int monthNum=currentMonth;
                lastMonth=currentMonth;
                int lastMonthCount = wv_month.getItemsCount();
                // 判断大小月及是否闰年,用来确定"日"的数据
                ...
                if (currentYear == startYear) {
                    currentMonthItem = wv_month.getCurrentItem() == 0 ? 0 :(lastMonthCount < 12?wv_month.getCurrentItem():wv_month.getCurrentItem() - (startMonth - 1));//记录上一次的item位置
                } else {
                    currentMonthItem = lastMonthCount < 12 ? wv_month.getCurrentItem() + startMonth - 1 : wv_month.getCurrentItem();
                }
        ...
        wv_month.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                lastMonth=currentMonth;//记录上一次选中的月份
        ...
        wv_day.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                if (currentYear == startYear && currentMonth == startMonth) {
                    day_num = index + startDay;
                }
                currentDay = day_num;
                int lastHourCount = wv_hours.getItemsCount();
                if (startYear == endYear && startMonth == endMonth && startDay == endDay) {
                    wv_hours.setAdapter(new NumericWheelAdapter(startHour, endHour));
                ...
                if (currentYear == startYear && currentMonth == startMonth && currentDay == startDay) {
                    currentHourItem = wv_hours.getCurrentItem() == 0 ? 0 : (lastHourCount < 24?wv_hours.getCurrentItem():wv_hours.getCurrentItem() - startHour);//记录上一次的item位置
                } else {//上次选中的年月日是限制的最小年月日，HourWheel上次的数据源就是24-startHour
                    currentHourItem = lastHourCount < 24 ? wv_hours.getCurrentItem() + startHour : wv_hours.getCurrentItem();
                }
                ...
                currentHour = wv_hours.getCurrentItem();
                //重置分钟
                int lastMinutesCount = wv_minutes.getItemsCount();
                if (startYear == endYear && startMonth == endMonth && startDay == endDay && startHour == endHour) {
                    wv_minutes.setAdapter(new NumericWheelAdapter(startMinute, endMinute));

                } else if (currentYear == startYear && currentMonth == startMonth && currentDay == startDay && currentHour == startHour) {
                    wv_minutes.setAdapter(new NumericWheelAdapter(startMinute, 59));

                } else if (currentYear == endYear && currentMonth == endMonth && currentDay == endDay && currentHour == endHour) {
                    wv_minutes.setAdapter(new NumericWheelAdapter(0, endMinute));
                ...
                int currentMinuteItem;
                if (currentYear == startYear && currentMonth == startMonth && currentDay == startDay && currentHour == startHour) {
                    currentMinuteItem = wv_minutes.getCurrentItem() == 0 ? 0 : (lastMinutesCount < 60?wv_minutes.getCurrentItem():wv_minutes.getCurrentItem() - startMinute);//记录上一次的item位置
                } else {
                    currentMinuteItem = lastMinutesCount < 60 ? wv_minutes.getCurrentItem() + startMinute : wv_minutes.getCurrentItem();
                }
        ...
        wv_hours.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                int hour_num = index;
                if (currentYear == startYear && currentMonth == startMonth && currentDay == startDay) {
                    hour_num = index + startHour;
                }
                currentHour = hour_num;
...
//                maxItem = endD;
        ...
        //
        int lastEndD;
        if (list_big.contains(String.valueOf(lastMonth))) {
            lastEndD = 31;
        } else if (list_little.contains(String.valueOf(lastMonth))) {
            lastEndD = 30;
        } else {
            if ((year_num % 4 == 0 && year_num % 100 != 0)
                    || year_num % 400 == 0) {
        ...
        int lastDayItemCount = wv_day.getItemsCount();
        ...
        if (currentYear == startYear && currentMonth == startMonth) {
            if (currentDay<startDay){
                ...
                currentItem = wv_day.getCurrentItem() == 0 ? 0 : (lastDayItemCount < lastEndD ? wv_day.getCurrentItem() : wv_day.getCurrentItem() - (startDay - 1));//记录上一次的item位置
            }
        } else {
            currentItem = lastDayItemCount < lastEndD ? wv_day.getCurrentItem() + startDay - 1 : wv_day.getCurrentItem();
        }
        ...
        //重置小时
        if (currentYear==startYear&&currentMonth==startMonth) {
            currentDay = startDay + currentItem;
        }else {
        ...
        int lastMinuteCount=wv_minutes.getItemsCount();
        if (startYear == endYear && startMonth == endMonth && startDay == endDay && startHour == endHour) {
            wv_minutes.setAdapter(new NumericWheelAdapter(startMinute, endMinute));
        } else if (currentYear == startYear && currentMonth == startMonth && currentDay == startDay && currentHour == startHour) {
            wv_minutes.setAdapter(new NumericWheelAdapter(startMinute, 59));
        ...
        int currentMinuteItem;
        if (currentYear == startYear && currentMonth == startMonth && currentDay == startDay && currentHour == startHour) {
            currentMinuteItem = wv_minutes.getCurrentItem() == 0 ? 0 :(lastMinuteCount< 60 ? wv_minutes.getCurrentItem() + startMinute : wv_minutes.getCurrentItem());
            if (currentMinuteItem<startMinute) {
                currentMinuteItem=0;
            }else if (wv_minutes.getItemsCount()<60){
                currentMinuteItem=currentMinuteItem-startMinute;
            }
        } else {
            currentMinuteItem = lastMinuteCount< 60 ? wv_minutes.getCurrentItem() + startMinute : wv_minutes.getCurrentItem();
        }
        if (currentMinuteItem > wv_minutes.getItemsCount() - 1) {
            currentMinuteItem = currentMinuteItem%wv_minutes.getItemsCount();
        }
        wv_minutes.setCurrentItem(currentMinuteItem);
    }
    ...
    public void setLabels(String label_year, String label_month, String label_day, String label_hours, String label_mins, String label_seconds) {
        ...
        if (label_year != null) {
            ...
            wv_year.setLabel(view.getContext().getString(R.string.view_pickerview_year));
        }
        if (label_month != null) {
            wv_month.setLabel(label_month);
        } else {
            wv_month.setLabel(view.getContext().getString(R.string.view_pickerview_month));
        }
        if (label_day != null) {
            wv_day.setLabel(label_day);
        } else {
            wv_day.setLabel(view.getContext().getString(R.string.view_pickerview_day));
        }
        if (label_hours != null) {
            wv_hours.setLabel(label_hours);
        } else {
            wv_hours.setLabel(view.getContext().getString(R.string.view_pickerview_hours));
        }
        if (label_mins != null) {
            wv_minutes.setLabel(label_mins);
        } else {
            wv_minutes.setLabel(view.getContext().getString(R.string.view_pickerview_minutes));
        }
        if (label_seconds != null) {
            wv_seconds.setLabel(label_seconds);
        } else {
            wv_seconds.setLabel(view.getContext().getString(R.string.view_pickerview_seconds));
        }
